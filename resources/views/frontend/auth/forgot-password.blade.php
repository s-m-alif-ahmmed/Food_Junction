...
@section('meta_infos')
    ...
    <meta name="description" content="Food Junction - Forgot Password">
    <meta name="keywords" content="Food Junction, Food, Junction, Dhaka, Sweets, Forgot Password">
@endsection
    ...
    <section class="login-signup-page py-5">
        <div class="auth-card" id="container">
            ...
            <div class="form-container sign-in">
                <form action="{{ route('forgot.password.otp') }}" method="POST" id="forgotPasswordForm">
                    @csrf
                    <h1 class="auth-title">Forgot Password</h1>

                    <span class="auth-subtitle">Enter the email address registered on your account to receive an OTP</span>

                    @if(session('message'))
                        <div class="alert alert-success w-100 py-2 text-center my-2" style="font-size: 13px;">{{ session('message') }}</div>
                    @endif
                    ...
                    <div class="input-field-group">
                        <input type="email" name="email" id="forgotEmail" placeholder="Registered Email Address" value="{{ old('email') }}" required autocomplete="email" />
                    </div>
                    @error('email')
