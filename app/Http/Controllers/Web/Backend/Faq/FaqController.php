...
class FaqController extends Controller
{
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    return '<div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                <a href="' . route('faqs.show', ['id' => $data->id]) . '" type="button" class="btn btn-secondary fs-14 text-white edit-icn" title="Edit">
                                    <i class="fe fe-eye"></i>
                                </a>
                                <a href="' . route('faqs.edit', ['id' => $data->id]) . '" type="button" class="btn btn-primary fs-14 text-white edit-icn" title="Edit">
    ...
    public function store(Request $request): RedirectResponse {
        try {
            ...
            return redirect()->route('faqs.index')->with('t-success', 'Updated successfully');
        } catch (Exception) {
            return redirect()->route('faqs.index')->with('t-success', 'Faq failed created.');
        }
    }

    public function show(int $id): View {
        $data = Faq::find($id);
        return view('backend.layouts.faq.detail', compact('data'));
    }
    ...
    public function update(Request $request, int $id): RedirectResponse {
        try {
            ...
            return redirect()->route('faqs.index')->with('t-success', 'Faq failed to update');
        }
    }
